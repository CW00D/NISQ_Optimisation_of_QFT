...
def get_circuit_fitnesses(target_states, circuits, chromosomes, initial_states, simulator=SIMULATOR):
    ...
    if batch_circuits:
        ...
        for count, i in enumerate(batch_indices):
            ...
            fidelity = compute_phase_fitness(circuit_states, target_states)
            # Note lambda_factor below is set to 0.005 but can be adjusted
            depth_penalty = 0.005 * circuits[i].depth()
